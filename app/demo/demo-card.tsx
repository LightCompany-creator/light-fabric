...
const ROLES: Role[] = [
  ...
  },
  {
    ...
    description: "Создаёт заказы и следит за всей веткой до сдачи.",
    capabilities: [
      "Создание главного заказа на производство",
      "Просмотр дерева заказа: цеха, статусы, план/факт",
      "Отслеживание прогресса по каждому заказу",
    ],
    icon: Briefcase,
  ...
  },
  {
    ...
    description: "Принимает заказы коммерческого директора → подзаказы по цехам.",
    capabilities: [
      "Приём заказа коммерческого директора или создание своего",
      "Разбивка заказа на подзаказы по цехам со сроками",
      "Разбор запросов на корректировку от цехов",
      "Закрытие заказа, когда закрыты все подзаказы (и повторное открытие)",
    ],
    icon: ListOrdered,
